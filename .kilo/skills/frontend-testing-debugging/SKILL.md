...
> **Kilo adaptation:** This repository uses Kilo's integrated browser and its existing Playwright setup instead of the Codex Browser plugin. Read "Browser available" as Kilo's browser tools being available; prefer the repository's existing E2E scripts and `npm run test:e2e` where they cover the flow, and record when you fall back.
