...
> **Kilo adaptation:** Installed standalone from the upstream Build Web Data Visualization plugin. Its specialist skills (D3, Canvas2D, WebGL/Three.js, geospatial, grammar-of-graphics, statistical, Gantt, UML, scrollytelling, testing, accessibility, reports) are not installed. Treat those pointers as named guidance to apply directly, and use the routing references in `references/` plus the foundations vendored under `references/foundations/`.
