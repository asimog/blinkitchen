...
> **Kilo adaptation:** Installed standalone from the upstream Build Web Data Visualization plugin. Shared theory is vendored under `../data-visualization/references/foundations/`. The sibling specialist skills (UML, scrollytelling, testing) are not installed; apply their named guidance directly.
...
If the main request is about screenshot testing, visual regression, mocked chart data, or E2E coverage strategy, apply the upstream `testing-data-visualizations` guidance first (not installed here) and pair back to this skill only for React- or Next-specific implementation details.
...
10. For editorial or infographic surfaces, keep the insight title, artifact mode, subtitle, annotation plan, source note, large-screen layout, mobile portrait layout, optional mobile landscape layout, and chart render layer as explicit component responsibilities. Use `../data-visualization/references/foundations/mobile-first-responsive-visualization.md` for touch, keyboard, visual viewport, spotty-connection, and device-capability decisions.
11. For scrollytelling or parallax, use the upstream `scrollytelling-and-parallax-data-visualization` guidance (not installed here) for the story contract. Model scenes as typed state rather than ad hoc scroll math. Each scene should define visible layers, annotation text, media assets, camera or transform state, trigger/progress range, static key frame, and reduced-motion fallback.
12. For advanced editorial, report, deck, generated-image, animation, or existing-page integration work, use `../data-visualization/references/foundations/meaning-preserving-visual-design-workflow.md` and `../data-visualization/references/foundations/mobile-first-responsive-visualization.md` before implementation. Apply the shared workflow for concept images, large-screen/mobile variants, approval or iteration, and the binding semantic design contract React must implement.
...
15. For interactive UML, ERD, dependency, state machine, flow, or architecture diagrams, use the upstream `uml-and-software-architecture-visualization` guidance (not installed here) first; React should own app state, panels, controls, routing, and persistence while the diagram layer owns layout/rendering details.
16. For operational workspaces, use `../data-visualization/references/foundations/operational-visualization-workspaces.md` before component decomposition. React should own shell state, command bars, drawers, rails, active summaries, URL state, and inspector synchronization while the visualization layer owns geometry, layout, and picking.
...
- Shared theory:
  - `../data-visualization/references/foundations/editorial-infographic-system.md`
  - `../data-visualization/references/foundations/art-directed-interactive-visual-stories.md`
  - `../data-visualization/references/foundations/meaning-preserving-visual-design-workflow.md`
  - `../data-visualization/references/foundations/task-abstraction-and-chart-selection.md`
  - `../data-visualization/references/foundations/perception-color-and-encoding.md`
  - `../data-visualization/references/foundations/shareable-state-and-persistence.md`
  - `../data-visualization/references/foundations/mobile-first-responsive-visualization.md`
  - `../data-visualization/references/foundations/operational-visualization-workspaces.md`
  - `../data-visualization/references/foundations/implementation-design-and-tradeoffs.md`
- Skill references:
  ...
  - Upstream specialist skills (`uml-and-software-architecture-visualization`, `scrollytelling-and-parallax-data-visualization`, `testing-data-visualizations`) are not installed; apply their named guidance directly.
