...
Installed standalone from the upstream Product Design plugin; the plugin router, user-context, Figma, and prototype skills are not installed here. Ground the audit in this repository: the product surfaces, flows, and existing design system are the source of truth. Capture with Kilo's integrated browser, falling back to Playwright when it is unavailable.

Do not inspect every surface in the codebase. Inspect only what the current task needs.
...
- Prefer Kilo's integrated browser; fall back to Playwright when it is unavailable or unstable, and record the fallback reason.
- If none of those can capture valid screenshots or control the flow, stop and report the blocker.

Browser capture order:

1. Open the browser surface before browser work.
