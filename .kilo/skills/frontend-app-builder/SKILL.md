...
> **Kilo adaptation:** This environment has no image-generation tool. When the workflow below calls for Image Gen concepts, produce the visual concept as an explicit design spec (layout, type scale, spacing, tokens, component families) and get approval before coding. Use Kilo's integrated browser for validation and this repository's Playwright setup as the fallback.
