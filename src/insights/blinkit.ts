...
/**
 * Spices and fats (salt, oil, turmeric) recur in almost every plan, so they win
 * any raw reuse count trivially. The lens surfaces the fresh and protein
 * ingredients where chaining is an actual planning choice.
 */
function isChainableIngredient(catalog: Catalog, ingredientId: string): boolean {
...
export function buildBlinkitInsights(
  ...
  const isChainable = (ingredientId: string): boolean =>
    isChainableIngredient(catalog, ingredientId);
  ...
  // Recorded history lives cumulatively in every later snapshot, so anything
  // derived from facts (decisions, meals, spoilage) is read from each
  // household's final snapshot only. Per-week projections above are summed
  // across snapshots, where each snapshot represents exactly one week.
  ...
  const archetypes: ArchetypeSummary[] = finals.map((state) => {
    ...
    const spoilageEvents = state.consumptionFacts.filter(
  ...
  const missingIngredients = [...missingCounts.entries()]
    ...
    .sort((a, b) => b.weeksMissing - a.weeksMissing || compareStrings(a.ingredientId, b.ingredientId))
    .slice(0, 10);

  const reusedIngredients = [...reuseCounts.entries()]
    .flatMap(([ingredientId, appearances]) =>
      isChainable(ingredientId)
        ? [{ ingredientId, name: nameOf(ingredientId), appearances }]
        : [],
    )
    .sort((a, b) => b.appearances - a.appearances || compareStrings(a.ingredientId, b.ingredientId))
    .slice(0, 10);
  ...
  const useSoonFrequency = [...useSoonCounts.entries()]
    ...
    .sort((a, b) => b.occurrences - a.occurrences || compareStrings(a.ingredientId, b.ingredientId))
    .slice(0, 8);
  ...
  const replenishmentSignals = [...replenishmentHouseholds.entries()]
    ...
    .sort((a, b) => b.households - a.households || compareStrings(a.ingredientId, b.ingredientId))
    .slice(0, 8);

  const cuisineSignals = [...cuisineCounts.entries()]
    .map(([cuisine, meals]) => ({ cuisine, meals }))
    .sort((a, b) => b.meals - a.meals || compareStrings(a.cuisine, b.cuisine));
