...
export function WeekChecklist({
  ...
  const rows: {
    ...
    action?: (variant: "primary" | "secondary") => ReactNode;
  }[] = [
    {
  ...
  const firstPending = rows.findIndex((row) => !row.done && row.action);
    ...
    <section className={styles.panel} aria-label="Your week, step by step">
      ...
      <ul className={styles.checklist}>
        {rows.map((row, index) => (
          <li
            key={row.label}
            className={`${styles.checklistRow} ${row.done ? styles.checklistRowDone : ""}`}
          >
            ...
            <span className={styles.checklistAction}>
              {row.action ? row.action(index === firstPending ? "primary" : "secondary") : null}
            </span>
          </li>
        ))}
      </ul>
